...
# Commented out to avoid wiping previous attendance records
...
# reset_attendance_table()  # ❌ REMOVE this to avoid deleting previous records
init_database()              # ✅ Keeps the tables without wiping data
...
def show_attendance_by_date():
    def fetch_and_display():
        ...
        if not from_date or not to_date:
            messagebox.showwarning("Input Error", "Please enter both from and to dates (YYYY-MM-DD).")
            return
        ...
        if not rows:
            messagebox.showinfo("No Records", f"No attendance records from {from_date} to {to_date}.")
        else:
            for row in rows:
                tree.insert('', 'end', values=row)

    win = tk.Toplevel(root)
    win.title("Attendance by Date Range")
    win.geometry("650x450")

    ttk.Label(win, text="From Date (YYYY-MM-DD):").pack(pady=5)
    from_date_entry = ttk.Entry(win, width=20)
    from_date_entry.pack()

    ttk.Label(win, text="To Date (YYYY-MM-DD):").pack(pady=5)
    to_date_entry = ttk.Entry(win, width=20)
    to_date_entry.pack()

    ttk.Button(win, text="Show Attendance", command=fetch_and_display).pack(pady=10)
...
ttk.Button(btns, text="Show Attendance by Date", command=show_attendance_by_date).grid(row=0, column=2, padx=10)
